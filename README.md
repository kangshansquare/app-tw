...
```
- [ ] 登录页面"记住我"
- [ ] 同一账号，只允许一个设备登录；后登录踢掉先登录
- [ ] 系统管理
```
导航添加一个系统管理页面，只有管理员登录时显示
1.添加、禁用、删除用户
2.其他类型的添加/删除等操作，比如中间件管理页面中，中间件所属业务线名称（业务线单独建一个表？）；IDC管理中添加机房、机柜等
```
- [ ] 一个考试系统页面
```
1.考试页面：学生信息、单选、多选、填空、名词解释、简答/论述，满分100分。
- 显示所有已录入科目，选择科目名称，点击“开始考试”，全屏显示、禁止复制粘贴、倒计时显示；点击“交卷”提交
2.题库录入：逐条录入、批量录入
- 科目名称、题型、答案、（分值）
```
